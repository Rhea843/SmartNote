          ...
          notes.map(note => (
            ...
            >
              <div className="flex item-center justify-between px-3 ">
               <h3 className="font-semibold text-[20px] ">{truncate(note.title, 20)}</h3>
               <button onClick={(e) =>{
                 e.stopPropagation();
             ...
             <p className="text-[16px] text-gray-900 font-normal line-clamp-2 mt-2 px-3 ">{truncate(note.content, 50)}</p>

             <p className="text-xs text-gray-900 mt-4 px-3 font-bold text-right">
               {formatDate(note.updated_at)}
             </p>

             {/* show menu */}
              {activeMenu === note.id && (
                <div className='bg-[#3A506A] rounded-lg shadow-full text-[#1A1B25] absolute top-12 lg:left-73 right-6 w-43 z-50 flex flex-col'>
